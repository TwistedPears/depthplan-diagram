...
pub(crate) fn send_user_message<T: UserEvent>(
  ...
  if current_thread().id() == context.main_thread_id {
    // Only the owner thread may clone/use/drop Tao's non-atomic window target.
    #[cfg(windows)]
    let window_target = context
      .main_thread
      .window_target
      .upgrade()
...
pub enum EventLoopWindowTargetMessage {
  ...
  MonitorFromPoint(f64, f64, Sender<Option<Monitor>>),
...
pub struct Wry<T: UserEvent> {
  context: Context<T>,
  // Handles hold Weak references, so this target is always dropped on its owner thread.
  #[cfg(windows)]
...
impl<T: UserEvent> RuntimeHandle<T> for WryHandle<T> {
  ...
  fn display_handle(
    ...
    // Windows display handles borrow no data; do not touch the event-loop target.
    #[cfg(windows)]
  ...
  fn monitor_from_point(&self, x: f64, y: f64) -> Option<Monitor> {
    ...
    let _ = send_user_message(
      &self.context,
      Message::EventLoopWindowTarget(EventLoopWindowTargetMessage::MonitorFromPoint(x, y, tx)),
    );
    rx.recv().unwrap_or_default()
  }
...
fn handle_user_message<T: UserEvent>(
  ...
  match message {
    ...
    Message::EventLoopWindowTarget(message) => match message {
      ...
      EventLoopWindowTargetMessage::MonitorFromPoint(x, y, sender) => {
        let _ = sender.send(
          event_loop
            .monitor_from_point(x, y)
            .map(|m| MonitorHandleWrapper(m).into()),
